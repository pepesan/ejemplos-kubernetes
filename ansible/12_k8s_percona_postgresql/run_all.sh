#!/usr/bin/env bash
# Despliega un clúster de PostgreSQL (Percona Operator) sobre Kubernetes HA con Longhorn y Cilium.
set -euo pipefail
...
run_playbook 12 12_desplegar_pg_operator.yml           "Desplegar el Percona Operator for PostgreSQL"
run_playbook 13 13_desplegar_pg_cluster.yml            "Desplegar el clúster PostgreSQL (Patroni)"
run_playbook 14 14_verificar_pg.yml                    "Verificar la replicación y el acceso externo"
...
PG_IP=$(kubectl --kubeconfig=kubeconfig.yaml -n postgres get svc pg-cluster-ha -o jsonpath='{.status.loadBalancer.ingress[0].ip}' 2>/dev/null || true)

{
  echo ""
  echo "════════════════════════════════════════════════════════════════"
  echo "  Clúster Kubernetes HA con PostgreSQL (Patroni) listo!"
  ...
  echo "  "
  if [ -n "$PG_IP" ]; then
    echo "  PostgreSQL (IP LB-IPAM de Cilium, vía pgBouncer): ${PG_IP}"
    echo "  Prueba: psql -h ${PG_IP} -U pg-cluster -d pg-cluster"
  fi
  if [ -f "pg_password.txt" ]; then
    echo "  Contraseña guardada en: $(pwd)/pg_password.txt"
  fi
  echo "  "
