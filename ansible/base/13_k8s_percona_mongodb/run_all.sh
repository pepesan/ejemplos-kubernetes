#!/usr/bin/env bash
# Despliega un clúster de MongoDB con sharding (Percona Operator) sobre Kubernetes HA con Longhorn y Cilium.
set -euo pipefail
...
run_playbook 02 02_crear_nodos.yml                     "Crear las máquinas virtuales LXD para el clúster (3 managers + 6 workers hiperconvergentes)"
...
run_playbook 12 12_desplegar_mongodb_operator.yml      "Desplegar el Percona Operator for MongoDB"
run_playbook 13 13_desplegar_mongodb_cluster.yml       "Desplegar el clúster MongoDB con sharding (2 shards de 3 réplicas)"
run_playbook 14 14_verificar_mongodb.yml               "Verificar el sharding y el acceso externo"
...
MONGODB_IP=$(kubectl --kubeconfig=kubeconfig.yaml -n mongodb get svc mongodb-cluster-mongos -o jsonpath='{.status.loadBalancer.ingress[0].ip}' 2>/dev/null || true)

{
  echo ""
  echo "════════════════════════════════════════════════════════════════"
  echo "  Clúster Kubernetes HA con MongoDB (sharding) listo!"
  ...
  echo "  Worker 4:  $(host_ip k8s-worker4)"
  echo "  Worker 5:  $(host_ip k8s-worker5)"
  echo "  Worker 6:  $(host_ip k8s-worker6)"
  echo "  "
  if [ -n "$MONGODB_IP" ]; then
    echo "  MongoDB (IP LB-IPAM de Cilium, vía mongos): ${MONGODB_IP}"
    echo "  Prueba: mongosh 'mongodb://databaseAdmin:<password>@${MONGODB_IP}:27017/admin'"
  fi
  if [ -f "mongodb_password.txt" ]; then
    echo "  Contraseña guardada en: $(pwd)/mongodb_password.txt"
  fi
  echo "  "
