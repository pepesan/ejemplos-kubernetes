#!/usr/bin/env bash
# Despliega un clúster de Percona XtraDB Cluster (Galera) sobre Kubernetes HA con Longhorn y Cilium.
set -euo pipefail
...
run_playbook 13 13_desplegar_pxc_cluster.yml           "Desplegar el clúster Percona XtraDB Cluster (Galera)"
...

{
  echo ""
  echo "════════════════════════════════════════════════════════════════"
  echo "  Clúster Kubernetes HA con Percona XtraDB Cluster (Galera) listo!"
  ...
  if [ -f "pxc_root_password.txt" ]; then
    echo "  Contraseña root guardada en: $(pwd)/pxc_root_password.txt"
  fi
  echo "  "
