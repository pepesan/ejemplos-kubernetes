...
Smoke-tests `lxd_host_bootstrap` against every distro it claims to support (`ansible/roles/lxd_host_bootstrap/meta/main.yml`: Ubuntu 24.04/26.04, Debian 12/13). For each distro:

1. Imports a container-format test image, if not already present (`00_prepare_images.yml`).
2. Creates a plain, throwaway LXD container and ensures `python3` is present — Debian's minimal container images don't ship it, Ubuntu's do (`01_provision.yml`).
