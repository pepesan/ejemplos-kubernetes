#!/usr/bin/env bash
# Ejecuta todos los playbooks para el clúster de Kubernetes HA con observabilidad completa en LXD.
set -euo pipefail
...

{
  echo ""
  echo "════════════════════════════════════════════════════════════════"
  echo "  Clúster Kubernetes HA con Observabilidad Completa listo!"
  ...
  echo "  Prometheus:        http://${VIP}:32090"
  echo "  Grafana:           http://${VIP}:32091 (usuario: admin)"
  if [ -f "grafana_admin_password.txt" ]; then
    echo "  Contraseña de Grafana guardada en: $(pwd)/grafana_admin_password.txt"
  fi
  echo "  "
